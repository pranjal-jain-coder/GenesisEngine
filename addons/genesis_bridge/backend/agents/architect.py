...
class PendingTaskUpdate(BaseModel):
    task_id: str
    new_description: str
    reason: str  # brief justification so user understands why it changed
...
# ---------------------------------------------------------------------------
# Prompt for generating additional tasks from a GDD diff
# ---------------------------------------------------------------------------

_GENERATE_ADDITIONAL_TASKS_PROMPT = """\
You are a game designer expanding an existing production task list because the Game Design Document has evolved.

## Current GDD (full):
{current_gdd_json}

## What Changed in the GDD Since Last Task Generation:
{gdd_diff}

## Existing Task Status

### COMPLETED tasks — already built. DO NOT touch these:
{completed_tasks_json}

### IN_PROGRESS tasks — currently executing. DO NOT touch these:
{in_progress_tasks_json}

### PENDING tasks — not yet started. You MAY propose description updates ONLY if a GDD change directly and materially affects their spec:
{pending_tasks_json}

## Your Job
1. Identify features, mechanics, enemies, UI elements, levels, or systems that appear in the CURRENT GDD but are NOT covered by any completed, in-progress, or pending task.
2. Write new tasks to cover those gaps. Number new tasks starting from TASK-{next_task_number:02d}.
3. If a GDD change invalidates or requires updating a PENDING task's specification (not just adding something new), propose an update with the corrected full description and a brief reason.
4. DO NOT propose updates to COMPLETED or IN_PROGRESS tasks.
5. If nothing is missing and no PENDING task needs updating, return empty lists — do not invent tasks.

## Rules (same as initial task generation)
...
### Full behavior spec for every new object
For each object introduced in a new task, define:
- **Lifecycle**: spawn position, spawn condition, spawn interval or trigger, every exit condition.
- **State machine**: every state by name, transition conditions, what keeps it in each state.
- **Movement**: starting position, direction, speed in pixels/second, every boundary condition.
- **Trigger → Action → Effect on others**.
- **Visual feedback**: exactly what the player sees — color, duration, size change.
- **Audio feedback**: what sound plays, when, roughly what it sounds like.

### Cross-system interactions
Re-state every interaction with objects from completed or pending tasks in full. Never assume the developer infers connections.

### Assets
Acquire visual and audio assets at the moment they are first needed. No placeholders. Describe style, colors, size.

### Concrete values
Give real numbers for all quantities — speeds, distances, timers, cooldowns, score values, HP.

### Done condition
End every new task with "✓ Done when:" followed by exactly what the developer will observe in-game.

## ANTI-PATTERNS — never write these
- "as expected", "standard behavior", "appropriate response", "handle accordingly", "etc."
- Any Godot node names (CharacterBody2D, Area2D) or function names (queue_free).

## OUTPUT FORMAT
Return a JSON object with exactly two keys:
- "new_tasks": list of task objects {{ "id": "TASK-XX", "description": "...", "status": "PENDING", "feedback": null }}. Empty list if nothing to add.
- "pending_task_updates": list of update objects {{ "task_id": "...", "new_description": "...", "reason": "..." }}. Empty list if no updates needed.
"""
...
class ArchitectAgent:
    ...
    # -------------------------------------------------------------------
    # GDD snapshot helpers (for generate_additional_tasks)
    # -------------------------------------------------------------------

    def _save_gdd_snapshot(self, project_path: str) -> None:
        """Persist the current GDD as a snapshot for future diff computation."""
        if not project_path or self.current_gdd is None:
            return
        try:
            snap_path = Path(project_path) / "tasks_gdd_snapshot.json"
            self.current_gdd.save_to_file(str(snap_path))
            logger.info(f"GDD snapshot saved to {snap_path}")
        except Exception as e:
            logger.error(f"Failed to save GDD snapshot: {e}")

    def _load_gdd_snapshot(self, project_path: str) -> Optional[GameDesignDocument]:
        """Load the GDD snapshot saved at last task generation, or None if missing."""
        snap_path = Path(project_path) / "tasks_gdd_snapshot.json"
        if not snap_path.exists():
            return None
        try:
            return GameDesignDocument.load_from_file(str(snap_path))
        except Exception as e:
            logger.error(f"Failed to load GDD snapshot: {e}")
            return None

    def _compute_gdd_diff(
        self, snapshot: GameDesignDocument, current: GameDesignDocument
    ) -> str:
        """Return a human-readable summary of what changed between two GDDs."""
        lines = []
        all_fields = (
            GameDesignDocument._STRING_FIELDS
            + GameDesignDocument._OBJECT_FIELDS
            + GameDesignDocument._LIST_FIELDS
        )
        for field in all_fields:
            old_val = getattr(snapshot, field, None)
            new_val = getattr(current, field, None)
            if old_val != new_val:
                lines.append(f"  [{field}]")
                lines.append(f"    BEFORE: {json.dumps(old_val, default=str)}")
                lines.append(f"    AFTER:  {json.dumps(new_val, default=str)}")
...
1. **Scope**: Each task covers a meaningful, self-contained piece of gameplay. Not a single object in isolation, not the entire game at once. Avoid doing too many things in a single task, and prefer focusing on one aspect in greater detail. As a rough guide, a task should introduce at most one new scene or one new core mechanic — if a task introduces both a new object and new input behavior and a new asset type, consider splitting it.

2. **Full behavior spec for every object**: For each object introduced or modified in the task, define:
   - **Lifecycle**: where and how it appears (spawn position, spawn condition, spawn interval or trigger), every state it can be in during its life, and every exit condition that removes it from the game (with exactly what happens on removal — sounds, score changes, effects).
   - **State machine**: if the object has multiple phases or modes, list every state explicitly by name, what keeps it in that state, and every condition that causes a transition to another state. Do not describe only the first phase — every phase must be fully specified.
   - **Movement completeness**: for any moving object, give its starting position, initial direction, speed in pixels/second, and what happens at EVERY boundary or threshold it can reach — screen edges, specific coordinate thresholds, proximity to another object, timer expiry. Every possible boundary must be addressed; leaving one unspecified means it will not be implemented.
   - **Trigger**: what causes it to act (which input, which collision, which timer, which condition)
   - **Action**: exactly what it does (moves at N px/s in direction X, fires every N seconds, disappears, decrements a counter by N)
   ...
   - **Destruction conditions**: list every object type and every event that can destroy or remove this object. Never assume the developer infers that a weapon can destroy an enemy — state it directly, including which projectile type, what happens to each participant on collision, and what game-state values change as a result.
   - **Visual feedback**: exactly what the player sees — color, duration, size change. E.g. "flashes white for 0.2 seconds then disappears"
...
6. **Concrete values**: Give real numbers for all quantities — number of enemies, lives, bullets, distances, speeds, timers, cooldowns, score values.
...
## ANTI-PATTERNS — never write these
- "The enemy behaves as expected" → say exactly what it does in each state
- "Handle the collision appropriately" → say what each collision causes for every participant
- "Add standard UI" → list every element, what value it shows, where on screen it sits
- "The player takes damage when hit" → say how much, describe the visual/audio feedback, state whether invincibility frames exist and for how long
- "Enemies pursue the player" → say at what speed, whether they accelerate, what happens when they reach the player, and what happens when they reach any screen boundary
- "The object moves in a pattern" → describe every segment of the path, every direction change, every speed change, every boundary condition that triggers a change in behavior, every state the object can be in
- "After reaching a certain point, the enemy changes behavior" → specify the exact coordinate or condition that triggers the change, and fully define every aspect of the new behavior
- "The enemy can be killed" → state explicitly which projectile types or events destroy it, what happens to the projectile on impact, what score or effect results, and what visual/audio plays
...
"""
        ...
        try:
            ...
            if project_path:
                ...
                self._save_gdd_snapshot(project_path)
...
1. **Scope**: Each task covers a meaningful, self-contained piece of gameplay. Not a single object in isolation, not the entire game at once. Avoid doing too many things in a single task, and prefer focusing on one aspect in greater detail. As a rough guide, a task should introduce at most one new scene or one new core mechanic.
...
7. **Completeness**: Verify that every mechanic and feature implied by the feedback is covered in the new tasks, and that nothing already built by the completed tasks is broken or contradicted. Before finalising the task list, check every mechanic, enemy type, item, UI element, win condition, and lose condition from the GDD is covered in at least one task. Edit the takss if necessary.
...
"""
    ...
    async def generate_additional_tasks(self, project_path: str = None) -> List[Task]:
        """Expand the task list based on GDD changes since the last task generation.

        Loads a GDD snapshot (saved alongside tasks.json), diffs it against the
        current GDD, and asks the LLM to produce:
          - new tasks for features/content not yet covered
          - optional description updates for existing PENDING tasks affected by the diff

        COMPLETED and IN_PROGRESS tasks are never touched.
        """
        if not self.tasks:
            logger.warning("generate_additional_tasks called but no tasks exist yet.")
            return []

        current_gdd = self.current_gdd
        if not current_gdd:
            logger.warning("generate_additional_tasks called but no GDD loaded.")
            return self.tasks

        snapshot = self._load_gdd_snapshot(project_path) if project_path else None
        if snapshot is None:
            diff_text = "(No previous snapshot — treating full GDD as the source of truth)"
        else:
            diff_text = self._compute_gdd_diff(snapshot, current_gdd)

        completed_tasks  = [t for t in self.tasks if t.status == TaskStatus.COMPLETED]
        in_progress_tasks = [t for t in self.tasks if t.status == TaskStatus.IN_PROGRESS]
        pending_tasks    = [t for t in self.tasks if t.status == TaskStatus.PENDING]

        next_num = len(self.tasks) + 1

        prompt = _GENERATE_ADDITIONAL_TASKS_PROMPT.format(
            current_gdd_json=current_gdd.to_json(),
            gdd_diff=diff_text,
            completed_tasks_json=json.dumps(
                [{"id": t.id, "description": t.description} for t in completed_tasks], indent=2
            ),
            in_progress_tasks_json=json.dumps(
                [{"id": t.id, "description": t.description} for t in in_progress_tasks], indent=2
            ),
            pending_tasks_json=json.dumps(
                [{"id": t.id, "description": t.description} for t in pending_tasks], indent=2
            ),
            next_task_number=next_num,
        )

        console.print("\n[bold green]📋 Generating additional tasks from GDD diff…[/bold green]")
        try:
            expansion: TaskExpansionResult = self.llm_task.generate_structured(
                prompt, TaskExpansionResult
            )

            # Apply updates to PENDING tasks only — guard against LLM touching others
            pending_id_set = {t.id for t in pending_tasks}
            for upd in expansion.pending_task_updates:
                if upd.task_id not in pending_id_set:
                    logger.warning(
                        f"Skipping update for task {upd.task_id} — not PENDING or not found."
                    )
                    continue
                for task in self.tasks:
                    if task.id == upd.task_id and task.status == TaskStatus.PENDING:
                        task.description = upd.new_description
                        break

            # Append new tasks, forcing PENDING status
            existing_ids = {t.id for t in self.tasks}
            for new_task in expansion.new_tasks:
                new_task.status = TaskStatus.PENDING
                if new_task.id in existing_ids:
                    import uuid
                    new_task.id = f"TASK-{uuid.uuid4().hex[:8].upper()}"
                self.tasks.append(new_task)
            ...
            console.print(
                f"  [green]✓ Added {len(expansion.new_tasks)} task(s), "
                f"updated {len(expansion.pending_task_updates)} pending task(s)[/green]\n"
            )
            return self.tasks

        except Exception as e:
            logger.error(f"Failed to generate additional tasks: {e}")
            return self.tasks
