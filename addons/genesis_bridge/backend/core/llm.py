...
class GeminiProvider(LLMProvider):
    ...
    def generate(self, prompt: str, system_instruction: str = None) -> str:
        ...
        last_error = Exception("Unknown error in generate")
    ...
    def generate_structured(self, prompt: str, response_model: Type[BaseModel]) -> BaseModel:
        ...
        max_retries = 3
        last_error = Exception("Unknown error in generate_structured")
...
Do not include any markdown formatting, code blocks, or explanations. Return only the raw JSON object."""
    ...
    def generate_with_tools(
        ...
        if not current_keys:
            raise ValueError("No Gemini API keys available.")
        max_attempts = len(current_keys)
        last_error = Exception("Unknown error in generate_with_tools")
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                if response.status_code >= 500:
                    ...
                    time.sleep(wait)
                    last_error = Exception(f"API Server Error {response.status_code}: {response.text}")
                    continue
