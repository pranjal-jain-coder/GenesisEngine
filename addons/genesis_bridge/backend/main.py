...
architect = None
studio_agent = None
project_review_agents: Dict[str, ReviewAgent] = {}
...
async def run_studio_agent_step(project_path: str, next_task: Any, websocket: WebSocket, initial_state: Dict = None):
    ...
    try:
        ...
        # Mark as completed (skip for ephemeral ad-hoc fixes)
        if not str(next_task.id).startswith("FIX-"):
            ...
            if not str(next_task.id).startswith("FIX-"):
                ...
                for _msg in reversed(_summary_state.values.get("messages", [])):
                    ...
                    if "[Coder]:" in _content:
                        _coder_log = _content
                        break
                # Use LLM to generate a concise architectural summary of the task
                logger.info(f"[StudioAgent] Generating architectural summary for task: {next_task.id}")
                
                summary_prompt = f"""Please read the execution log below for the task that was just completed.
Task: {next_task.description}

Execution Log:
{_coder_log}

Please generate a 2-3 sentence purely factual, technical summary of what was actually built or modified in the project (e.g. "Created Player.tscn and attached player.gd. Added health and speed variables. Instanced Player into Main.tscn."). Do NOT include conversational filler, only the concrete architectural facts. Do not list failed attempts, only the final successful outcome."""

                import asyncio
                summary_text = await asyncio.to_thread(
                    llm_flash.generate,
                    summary_prompt,
                    "You are a technical architect documenting changes to a Godot 4 project."
                )
                ...
                project_task_summaries[project_path].append({
                    ...
                    "key_actions": [summary_text.strip()],
                })
        ...
        if not str(next_task.id).startswith("FIX-"):
            ...
            GitManager.commit_state(project_path, f"Completed Ad-Hoc Fix: {next_task.description}")
        ...
        # On failure, revert the entire project to the state before the task started.
        logger.info("[StudioAgent] Attempting project revert to pre-task snapshot.")
        commits = GitManager.get_commits(project_path)
        target_commit = None
        
        target_prefix = f"State before Ad-Hoc Fix: {next_task.description}" if str(next_task.id).startswith("FIX-") else f"State before Task: {next_task.description}"
        
        for commit in commits:
            message = commit.get("message", "")
            if message.startswith(target_prefix):
                target_commit = commit["hash"]
                break
                
        # Fallback to previous behavior if the exact start commit isn't found
                        ...
                        if mode == "planning":
                            ...
                            if action == "execute_fix":
                                ...
                                if not desc:
                                    await websocket.send_text(json.dumps({
                                        "type": "error",
                                        "content": "No fix description provided."
                                    }))
                                    continue
                                
                                import uuid
                                from agents.architect import Task
                                ephemeral_task = Task(
                                    id=f"FIX-{uuid.uuid4().hex[:8].upper()}",
                                    description=desc,
                                    status=TaskStatus.IN_PROGRESS
                                )
                                logger.info(f"[Review] Starting ad-hoc fix: {desc}")
                                GitManager.commit_state(project_path, f"State before Ad-Hoc Fix: {desc}")
                                pre_fix_hash = GitManager.get_head_hash(project_path)

                                if project_path in project_review_agents:
                                    project_review_agents[project_path].reset()

                                await websocket.send_text(json.dumps({
                                    "type": "task_started",
                                    "task_id": ephemeral_task.id,
                                    "content": f"Executing Fix: {desc}",
                                    "pre_fix_hash": pre_fix_hash,
                                }))
                                
                                current_gdd = architect.get_current_gdd()
                                
                                # Build prior-work context from ALL tasks
                                all_tasks = [t for t in architect.tasks]
                                ...
                                if all_tasks:
                                    ...
                                    count = 1
                                    for t in all_tasks:
                                        entry = f"{count}. {t.description} (Status: {t.status.value})"
                                        ...
                                        count += 1
                                    completed_tasks_ctx = (
                                        "Previously planned/completed tasks context:\n"
                                        + "\n".join(_ctx_lines)
                                    )
                                else:
                                    completed_tasks_ctx = ""
                                ...
                                await run_studio_agent_step(project_path, ephemeral_task, websocket, initial_state)
                            
                            elif action == "reset_review":
                                if project_path in project_review_agents:
                                    project_review_agents[project_path].reset()
                                # No-op response — UI already cleared itself.

                            elif action == "revert_last_fix":
                                commit_hash = msg.get("commit_hash", "")
                                if not commit_hash:
                                    await websocket.send_text(json.dumps({
                                        "type": "error",
                                        "content": "No commit hash provided for revert."
                                    }))
                                    continue
                                ...
                                if revert_success:
                                    ...
                                    await websocket.send_text(json.dumps({
                                        "type": "project_reverted",
                                        "content": "Fix reverted successfully.",
                                        "tasks": [t.model_dump(mode='json') for t in architect.tasks],
                                        "gdd": json.loads(current_gdd.to_json()) if current_gdd else {}
                                    }))
                                else:
                                    await websocket.send_text(json.dumps({
                                        "type": "error",
                                        "content": "Failed to revert fix."
                                    }))

                            else:
                                # Just chat
                                if project_path not in project_review_agents:
                                    project_review_agents[project_path] = ReviewAgent(llm_flash)
                                result = await project_review_agents[project_path].chat(user_message, project_path, architect, manager)
                                response_msg = {
                                    "type": "chat_response",
                                    "content": result.get("response", ""),
                                }
                                ...
                                # If the agent finalized a task, send the ready event
                                if result.get("task_description"):
                                    await websocket.send_text(json.dumps({
                                        "type": "review_task_ready",
                            ...
                            # 10. Generate Additional Tasks
                            elif action == "generate_additional_tasks":
                                current_gdd = architect.get_current_gdd()
                                if not current_gdd:
                                    await websocket.send_text(json.dumps({
                                        "type": "error",
                                        "content": "No Game Design Document found. Cannot expand tasks."
                                    }))
                                elif not architect.tasks:
                                    await websocket.send_text(json.dumps({
                                        "type": "error",
                                        "content": "No tasks exist yet. Use 'Generate Tasks' first."
                                    }))
                                else:
                                    await websocket.send_text(json.dumps({
                                        "type": "status",
                                        "content": "Analyzing GDD changes and generating additional tasks..."
                                    }))
                                    updated_tasks = await architect.generate_additional_tasks(project_path)
                                    await websocket.send_text(json.dumps({
                                        "type": "task_list",
                                        "tasks": [t.model_dump(mode='json') for t in updated_tasks]
                                    }))

                            # 11. Task Verification (Disabled)
