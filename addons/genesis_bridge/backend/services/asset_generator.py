...
class AssetGenerator:
            ...
            if img:
                ...
                if transparent_bg:
                    ...
                    if config.IMAGE_PROVIDER == "local":
                        ...
                        # Cloud providers can return arbitrary plain backgrounds; use
                        # flood-fill edge removal only for that path.
                ...
                # Post-process: resize to target dimensions while preserving color fidelity.
                # Local SD often generates at 512+ and then we shrink to small sprite sizes.
                # NEAREST on large downscale factors causes severe color garbling.
                if img.size != (width, height):
                    img = self._resize_generated_image(img, width, height, style)
        ...
        if self.image_provider:
            try:
                ...
                if img:
                    ...
                    if img.size != (total_w, total_h):
                        img = self._resize_generated_image(img, total_w, total_h, style)
    ...
    def _resize_generated_image(
        ...
        """
        Resize generated images with safe defaults:
          - Upscale pixel art with NEAREST.
          - Downscale pixel art with BOX for color-safe reduction.
          - Use LANCZOS for non-pixel-art.
        """
        src_w, src_h = img.size
        if src_w == target_w and src_h == target_h:
            return img

        is_upscale = target_w > src_w or target_h > src_h
        if style == SpriteStyle.PIXEL_ART:
            if is_upscale:
                resample = Image.Resampling.NEAREST
            else:
                # BOX avoids the color speckling produced by NEAREST on large reductions.
    ...
    def _remove_chroma_key_background(self, img: Image.Image, tolerance: int = 70) -> Image.Image:
        ...
        Uses soft alpha blending for anti-aliased edge pixels.
        Only background pixels CONNECTED to the image border are removed, so
        interior sprite colors are preserved even if they are magenta-like.
        """
        ...
        def matte_distance(r: int, g: int, b: int) -> int:
        ...
        def is_candidate(r: int, g: int, b: int, a: int) -> bool:
            if a <= 0:
                return False
            # Limit to hues that are plausibly part of the magenta matte.
            if not (r >= 140 and b >= 140 and g <= 170):
                return False
            return matte_distance(r, g, b) < (tolerance + softness)

        visited = [[False] * w for _ in range(h)]
        q: deque[Tuple[int, int]] = deque()

        # Seed from all border pixels that look like matte background.
        for x in range(w):
            for y in (0, h - 1):
                r, g, b, a = px[x, y]
        ...
        # Remove only border-connected matte regions.
        while q:
            x, y = q.popleft()
            ...
            r, g, b, a = px[x, y]
            if not is_candidate(r, g, b, a):
                continue

            distance = matte_distance(r, g, b)
            if distance < tolerance:
                px[x, y] = (0, 0, 0, 0)
            else:
                # Edge pixel - fade alpha only; keep RGB intact.
